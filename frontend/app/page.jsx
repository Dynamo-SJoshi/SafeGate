      ...
      <section className="hero">
        ...
        <form className="uploadCard" onSubmit={handleAnalyzeUrl} key="url-form">
          ...
          {typeof window !== "undefined" && 
           ...
           ((urlResult?.error && urlResult.error.includes("Remote file exceeds the SafeGate fetch limit.")) ||
            (urlResult?.static_analysis?.error && urlResult.static_analysis.error.includes("Remote file exceeds the SafeGate fetch limit."))) && (
            <div style={{
              marginTop: "20px",
              padding: "18px",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                <div>
                  ...
                  <code style={{ background: "rgba(0, 0, 0, 0.4)", color: "#e2e8f0", padding: "6px 10px", borderRadius: "6px", display: "block", fontFamily: "monospace", fontSize: "0.8rem", border: "1px solid rgba(255,255,255,0.05)" }}>
                    docker compose exec backend clamscan /app/storage/{
                      (() => {
                        try {
                          const parsed = new URL(urlInput);
                          const pathname = parsed.pathname;
                          const name = pathname.substring(pathname.lastIndexOf('/') + 1);
                          return name || "your_file";
                        } catch (e) {
                          return "your_file";
                        }
                      })()
                ...
                <div>
                  ...
                  <code style={{ background: "rgba(0, 0, 0, 0.4)", color: "#e2e8f0", padding: "6px 10px", borderRadius: "6px", display: "block", fontFamily: "monospace", fontSize: "0.8rem", border: "1px solid rgba(255,255,255,0.05)" }}>
                    docker compose exec backend yara /app/analyzers/rules.yar /app/storage/{
                      (() => {
                        try {
                          const parsed = new URL(urlInput);
                          const pathname = parsed.pathname;
                ...
                <div>
                  ...
                  <code style={{ background: "rgba(0, 0, 0, 0.4)", color: "#e2e8f0", padding: "6px 10px", borderRadius: "6px", display: "block", fontFamily: "monospace", fontSize: "0.8rem", border: "1px solid rgba(255,255,255,0.05)" }}>
                    docker run --rm --network none --memory 2g -v C:\Users\DELL\Downloads\SafeGate\storage:/sandbox python:3.10-slim python /sandbox/{
                      (() => {
                        try {
                          const parsed = new URL(urlInput);
                          const pathname = parsed.pathname;
